(() => {
  ...
  function calcValues(values, currentYOffset) {
    ...
    if (values.length === 3) {
      ...
      const partScrollEnd = values[2].end * scrollHeight; // ㄸ끝나는스크롤위치
      const partScrollHeight = partScrollEnd - partScrollStart; //총 스크롤 길이

      if (
        currentYOffset >= partScrollStart && //시작점을 지나고
        currentYOffset <= partScrollEnd //마지막점을 지나지 않아씅면
      ) {
        rv =
          values[0] +
          ((currentYOffset - partScrollStart) / partScrollHeight) * //애니메이션 내에서 이동한 비율에
      ...
      // start, end값이 없으면
      rv = scrollRatio * (values[1] - values[0]) + values[0]; //전체 씬의 스크롤ratio를 반영해서 적용,
    }
